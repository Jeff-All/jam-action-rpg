...
func pick_ability(cur: EnemyUI, _battle: BattleUI) -> Array:
	if !find_target_intimidating_howl(cur).has_buff(ability_inimidating_howl.buff):
		var roll = randi_range(0,5)
		if roll >= 5 - intimidating_howl_roll_adjustment:
			intimidating_howl_roll_adjustment = -3
...
func find_target_intimidating_howl(cur: EnemyUI) -> PCUI:
