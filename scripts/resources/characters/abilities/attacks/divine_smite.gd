...
func inflict(_ability_button: AbilityButton, source: PCUI, target):
	...
	if shield <= 0:
		source.add_shield((damage * damage_to_shield + source._character.attributes[CharacterCampaign.Attributes.MAGIC].adjusted) + base_shield)
	else:
		damage += (shield * shield_to_damage) + source._character.attributes[CharacterCampaign.Attributes.MAGIC].adjusted
