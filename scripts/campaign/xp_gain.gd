...
func _character_xp_window_on_level_up(character: CharacterCampaign):
	if level_up.set_up(character, character._class.levels[character.level + 1].abilities, character._class.levels[character.level + 1].traits):
		level_up.visible = true
	else: _level_up_on_finish_level_up(character)

func _level_up_on_finish_level_up(character: CharacterCampaign):
	level_up.visible = false
	for cur in xp_windows:
