...
func take_damage(value: float, ignore_armor: bool = false):
	...
	if value > 0:
		...
		if armor > 0 && durability > 0 && !ignore_armor:
			...
			print("dur_damage: %s = snap(%s * %s / %s * %s)" % [dur_damage, _min, _min, armor, durability_scaling])
...
func apply_buff(buff: BuffPC, caster):
	...
	for cur in buffs:
		if cur._buff == null:
			cur.start_buff(self, buff, caster)
			return

func overwrite_buff(buff: BuffPC, caster) -> bool:
	for cur in buffs:
		if cur.buff != null:
			if cur.buff.name == buff.name:
				if buff.stackable:
